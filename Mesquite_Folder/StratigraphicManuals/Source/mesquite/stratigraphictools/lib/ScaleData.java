...
//TODO: support new versions of copyData and getStatesSummary from Mesquite 2. 01
/* ======================================================================== */
...
public class ScaleData extends CharacterData implements ItemContainer {
	private Double2DArray firstMatrix;
	protected Vector matrices; // Used to be private, change in 2005 to implement ScaleData.class in pacquage ScaleMatrix
	//private Vector matrices;
	private static Vector defaultModels;
	static {
		defaultModels= new Vector(2);
	}
	public ScaleData(CharMatrixManager manager, int numTaxa, int numChars, Taxa taxa){
	...
	public  void copyData(CharacterData sourceData){
		if (sourceData.getNumTaxa()==getNumTaxa() && sourceData.getNumChars() == getNumChars()) {
			ScaleData cD = (ScaleData)sourceData;
			int nItems = getNumItems();
			NameReference nr = NameReference.getNameReference(getItemName(0));
			setItemReference(0, nr);
			for (int i=0; i<nItems; i++){
				Double2DArray imatrix = null;
				if (i==0)
					imatrix = cD.getItem(i);
				else 
					imatrix = cD.establishItem(getItemName(i));
	...
	public CharacterData cloneData(){
		ScaleData cD = new ScaleData(getMatrixManager(), getNumTaxa(), getNumChars(), getTaxa());
		int nItems = getNumItems();
		NameReference nr = NameReference.getNameReference(getItemName(0));
		setItemReference(0, nr);
		for (int i=0; i<nItems; i++){
	...
	public int getNumTaxa(){
		if (disposed)
			return 0;
		int n = super.getNumTaxa();
		if (firstMatrix == null)
			MesquiteMessage.warnProgrammer("Scale data with null internal matrix. getNumTaxa() = " + n);
		else if (firstMatrix.getSizeT() != n)
			MesquiteMessage.warnProgrammer("Scale matrix with incorrect record of number of Period. getNumTaxa() = " + n + " firstMatrix.getSizeT() " + firstMatrix.getSizeT());
		return n;
	}
	public int getNumChars(){
		if (disposed)
			return 0;
		int n = super.getNumChars();
		if (firstMatrix == null)
			MesquiteMessage.warnProgrammer("Scale data with null internal matrix. getNumChar() = " + n  + " nAdd = " + nAdd + " nDel = " + nDel + " nMove = " + nMove);
		else if (firstMatrix.getSizeC() !=n)
			MesquiteMessage.warnProgrammer("Scale matrix with incorrect record of number of characters. getNumChar() = " + n + " firstMatrix.getSizeC() " + firstMatrix.getSizeC() + " nAdd = " + nAdd + " nDel = " + nDel + " nMove = " + nMove);
		return n;
	}
	/*..........................................ContinuousData................*/
	/** Removes item.*/
		...
		for (int item = 0; item<getNumItems(); item++){
			...
			for (int it=0; it<numTaxa; it++) {
				...
				for (int ic=0; ic<num; ic++)
						newMatrix.setValue(ic + starting +1,it,ScaleState.unassigned); //filling new part with missing data
		...
		for (int item = 0; item<getNumItems(); item++){
			...
			for (int ic=0; ic<numChars; ic++){
				...
				for (int it=0; it<num; it++) 
					//newMatrix.setValue(ic,it + starting +1,ScaleState.unassigned); //filling new part with missing data
					newMatrix.setValue(ic,it + starting +1,(double)0);
	...
	public void tradeStatesBetweenCharacters(int ic, int ic2, int it) { 
   		if (ic<numChars && ic2<numChars &&  it <numTaxa) 
			return;
		for (int item=0; item<getNumItems(); item++) {
			Double2DArray matrix = getItem(item);
	...
	/*..........................................ContinuousData................*/
	private String statesToString(int ic, int it, boolean forDisplay) {
		String s = "";
		boolean first = true;
		for (int item = 0; item<getNumItems(); item++) {
			if (!first)
	...
	/*..........................................ContinuousData................*/
   	public CharacterDistribution getCharacterDistribution(int ic){
   		ScaleEmbedded states =new ScaleEmbedded(this, ic);
		return states;
   	}
	/*..........................................  ContinuousData  ..................................................*/
   	/** Fills matrix with data from passed StsOfCharacters object.*/
   	public void setMatrix(MCharactersDistribution states){ 
   		if (!(states instanceof MScaleDistribution))
   			return;
		setDirty(true);
   		MScaleDistribution cStates = (MScaleDistribution)states;
   		setItemsAs(cStates);
   		for (int i = 0; i< getNumItems(); i++) {
	...
	/**get copy of matrix and return as MCharactersDistribution */
   	public MCharactersDistribution getMCharactersDistribution(){
   		MScaleEmbedded states = new MScaleEmbedded(this);
		return states;
   	}
	/*..........................................ContinuousData................*/
   	public CharacterDistribution makeCharacterDistribution(){
   		ScaleAdjustable c = new ScaleAdjustable(getTaxa(), numTaxa);
   		c.setItemsAs(this);
	...
	/*..........................................ContinuousData................*/
   	public CharacterData makeCharacterData() {
   		ScaleData data = new ScaleData(getMatrixManager(), getNumTaxa(), getNumChars(), getTaxa());
   		data.setItemsAs(this);
   		return data;
   	}
	/*..........................................ContinuousData................*/
   	public CharacterData makeCharacterData(int ntaxa, int nchars) {
   		ScaleData data = new ScaleData(getMatrixManager(), ntaxa, nchars,  getTaxa());
   		data.setItemsAs(this);
   		return data;
   	}
   	public CharacterData makeCharacterData(CharMatrixManager manager, Taxa taxa){ 
   		ScaleData data =  new ScaleData(getMatrixManager(), taxa.getNumTaxa(), 0, taxa);
   		data.setItemsAs(this);
   		return data;
   	}
	/*..........................................ContinuousData................*/
	...
	/*..........................................ContinuousData................*/
   	...
   	public  void setState(int ic, int it, CharacterState cs){
   		...
   		if (cs !=null && cs instanceof ScaleState) {
   			ScaleState contS = (ScaleState)cs;
   			if (contS.getNumItems()==1 && getNumItems()==1 && (contS.getItemReference(0) == null || getItemReference(0) == null || contS.getItemReference(0) == getItemReference(0))){
	...
	/*..........................................ContinuousData................*/
   	public void setState(int ic, int it, String s){
		  MesquiteMessage.warnProgrammer("Error: simple set state used in scale data");
	...
	assume that whole string is relevant*/
   	public int setState(int ic, int it, Parser parser, boolean fromEditor, MesquiteString result){ 
   		if (ic>=numChars || ic < 0 || it>=numTaxa ||it<0) {
   			if (result!=null)
   				result.setValue("Time or Period number out of bounds in setState " + ic + "  " + it);
		...
		while (!done){
			...
			if (StringUtil.blank(t)|| ")".equals(t)) {
				...
				double d = MesquiteDouble.fromString(s, pos);
 				if (d!=MesquiteDouble.impossible) {
					if (d != getState(ic, it, item)) {
	   				...
	   				if (result != null) {
	   					result.setValue("Illegal state value for scale Time");
	   					return ERROR;
	   				}
	   			}
	   			if (done)
	...
	/*..........................................ContinuousData................*/
   	...
   	public  CharacterState getCharacterState(CharacterState cs, int ic, int it){
   		if (cs==null || cs.getClass()!=ScaleState.class) {
   			cs =new ScaleState();
		}
   		((ScaleState)cs).setItemsAs(this);
   		for (int i=0; i< getNumItems(); i++) {
   			((ScaleState)cs).setValue(i, getState(ic, it, i));
   		}
   		return cs; 
   	}
	...
	public String getStatesSummary(int ic){
			...
			for (int it=0; it<numTaxa; it++) {
				state = (ScaleState)getCharacterState(state, ic, it);
	...
	/*..........................................ContinuousData................*/
   	...
   	public  boolean isInapplicable(int ic, int it){
   		if (getNumItems()==1)
   			return firstMatrix.getValue(ic,it)== ScaleState.inapplicable; 
   		else {
	   		for (int i=0; i< getNumItems(); i++) {
	   			if (!ScaleState.isInapplicable(getState(ic, it, i)));
	   				return false;
	   		}
	   		return true;
   		}
	...
	/*..........................................ContinuousData................*/
   	...
   	public  boolean isUnassigned(int ic, int it){
   		if (getNumItems()==1)
   			return firstMatrix.getValue(ic,it)== ScaleState.unassigned; 
   		else {
	   		for (int i=0; i< getNumItems(); i++) {
	   			if (!ScaleState.isUnassigned(getState(ic, it, i)));
	   				return false;
	   		}
	   		return true;
   		}
	...
	/*..........................................ContinuousData................*/
   	public String toString(){
   		return "Scale data matrix id: " + getID() + "; Period: " + numTaxa + "; items " + getNumItems() + "; name: " + getName();
   	}
}
